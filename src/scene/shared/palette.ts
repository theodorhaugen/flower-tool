...
export const PALETTES: readonly ColorPalette[] = [
  {
    ...
    deepShade: '#2B1E12',
    paleLight: '#F5F9F8',
    // Known outlier: this palette is deliberately the registry's own
    // "genuinely near-white" extreme (see the class docstring above), so it
    // has the least headroom of any palette before the general exposure
    // brightening (SceneLighting.tsx/paletteColors.ts, raised for an
    // overall-too-dark/moody complaint) blows out a sky-heavy composition —
    // confirmed on seed 42. A same-direction `atmosphereScale` reduction
    // (the lever Greenhouse bloom uses the opposite way, for under-
    // exposure) was tried and measurably made no difference, so the actual
    // cause isn't the haze/volumetric contribution this scales — left
    // alone rather than guessing further at real per-render verification
    // cost; every other palette reads correctly with the current fix.
  },
  {
    name: 'Poppy petal',
  ...
  },
  {
    ...
    // doubles as the petal family's near-black extreme
    // (flowerField/palette.ts's `petalAnchors`). Left dark deliberately —
    // this is the one role in the family meant to stay near-black, so it's
    // exempt from the lightness pass above.
    deepShade: '#3D2A0E',
  ...
  },
  {
    ...
    // `petalTertiary` (the palest of the three petal roles) stays close to
    // it, with `petalPrimary`/`petalSecondary` pushed rosier/more saturated
    // ahead of it. Same reasoning as every other palette's petal anchors
    // (class docstring, and directly the lesson from Daisies' own
    ...
    // doubles as the petal family's near-black extreme
    // (flowerField/palette.ts's `petalAnchors`). Left dark deliberately,
    // same exemption as Amber meadow's own `deepShade`.
    deepShade: '#3D2013',
  ...
  },
  {
    ...
    foliagePrimary: '#1E4640',
    foliageSecondary: '#2E7D5E',
    // The `#CA3C25` reference is dark/saturated enough that it was first
    // used directly as `petalPrimary`, with `petalSecondary`/`petalTertiary`
